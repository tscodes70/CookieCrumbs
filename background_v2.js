...
importScripts('lib/datastore-idb.js');
importScripts('lib/libp2p-bootstrap.js');
importScripts('lib/libp2p-identify.js');
importScripts('lib/libp2p-noise.js');
importScripts('lib/libp2p-websockets.js');
importScripts('lib/libp2p-yamux.js');
...
const { IDBDatastore } = DatastoreIdb;
const { noise } = ChainsafeLibp2PNoise;
const { yamux } = ChainsafeLibp2PYamux;
const { bootstrap } = Libp2PBootstrap;
const { identify } = Libp2PIdentify;
const { webSockets } = Libp2PWebsockets;
...
async function initializeHelia() {
    try {
        ...
        // Initialize libp2p with your desired configuration (using bootstrap nodes, etc.)
        const libp2p = await createLibp2p({
            datastore, // now using the persistent datastore
            addresses: {
                listen: [
        ...
        // Create the Helia node with the persistent datastore, blockstore, and libp2p instance
